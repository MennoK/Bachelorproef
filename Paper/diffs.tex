...
%DIF DEL paper.tex      Fri May 29 15:04:38 2015
%DIF ADD paper_v2.tex   Fri May 29 14:48:21 2015
...
Het herkennen van menselijke bewegingen is een belangrijk onderdeel van `context aware computing'. Uit de metingen van de accelerometer en gyroscoop van een smartphone kan de activiteit van de gebruiker bepaald worden. In het eerste deel van \DIFdelbegin \DIFdel{ons onderzoek zoeken we }\DIFdelend \DIFaddbegin \DIFadd{het onderzoek wordt gezocht }\DIFaddend naar modellen om afzonderlijke activiteiten te herkennen. Het nauwkeurigste model wordt gevonden met de classificatiemethode Random Forests met een accuraatheid van 93\%. Vervolgens \DIFdelbegin \DIFdel{gebruiken we dit model }\DIFdelend \DIFaddbegin \DIFadd{wordt dit model gebruikt }\DIFaddend om een sequentie van verschillende bewegingen te evalueren. \DIFdelbegin \DIFdel{We ontwikkelen hiervoor een algoritme }\DIFdelend \DIFaddbegin \DIFadd{Hiervoor wordt een algoritme ontwikkeld }\DIFaddend dat de sequentie in opeenvolgende tijdsvensters van een aantal seconden knipt, die ook met een bepaald percentage kunnen overlappen. Uit experimenten blijkt dat hiervoor een tijdsvenstergrootte van vier tot zes seconden
\DIFdelbegin \DIFdel{met een overlapping van 75}%DIFDELCMD < \% %%%
\DIFdelend %DIF >  met een overlapping van 75\%    --> dit misschien beter niet zeggen, want meer overlapping = nauwkeuriger...
...
Zo werd \DIFdelbegin \DIFdel{er }\DIFdelend in de paper \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Ravi and others:act.recogn.}
}%DIFAUXCMD
}\DIFdelend van Ravi, Dandekar, Mysore en Littman \DIFdelbegin \DIFdel{onderzocht }\DIFdelend \DIFaddbegin \DIFadd{onderzoek gedaan }\DIFaddend naar herkenning van afzonderlijke activiteiten met behulp van gegevens opgemeten door een externe accelerometer\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Ravi and others:act.recogn.}
}%DIFAUXCMD
}\DIFaddend .  Zij maakten gebruik van meta-level classifiers om modellen te genereren.

Ook \DIFdelbegin \DIFdel{andere papers, zoals~\mbox{%DIFAUXCMD
\cite{6dmotion}
}%DIFAUXCMD
maken gebruik }\DIFdelend \DIFaddbegin \DIFadd{in andere papers werd er gebruik gemaakt }\DIFaddend van externe accelerometers, maar zij verschillen dan in het gebruik van classifiers. Zo hebben Chen, Alregib en Juang HMM classifiers onderzocht\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{6dmotion}
}%DIFAUXCMD
}\DIFaddend .

In ons onderzoek worden gegevens opgemeten \DIFdelbegin \DIFdel{waarbij de meetapparatuur zich bevindt rondom de bekken . In ~\mbox{%DIFAUXCMD
\cite{act.rec.}
}%DIFAUXCMD
deden }\DIFdelend \DIFaddbegin \DIFadd{met meetapparatuur die zich rondom het bekken bevindt. }\DIFaddend Casale, Pujol en Radeva \DIFaddbegin \DIFadd{deden }\DIFaddend gegevensopmetingen met een accelerometer rondom de borstzone en vonden een nauwkeurig model met behulp van Random Forests\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{act.rec.}
}%DIFAUXCMD
}\DIFaddend .

\DIFdelbegin \DIFdel{De meeste van deze papers hebben zich gefocust }\DIFdelend \DIFaddbegin \DIFadd{In deze papers lag de focus vooral }\DIFaddend op het herkennen van afzonderlijke activiteiten. Deze paper focust zich naast de afzonderlijke activiteiten ook op het herkennen van sequenties van activiteiten.
...
Voor elke meting werd de applicatie gestart alvorens de smartphone in de broekzak gestoken werd en gestopt na het uithalen. Daarom bevat het begin en einde van elke meting enkele seconden die niet tot de gemeten activiteit horen. Om het fout labelen te vermijden, werd na elke meting een stuk van \DIFdelbegin \DIFdel{de start en het }\DIFdelend \DIFaddbegin \DIFadd{het begin en }\DIFaddend einde van het logbestand weggeknipt, zodat elke meting exact \'e\'en activiteit bevat van vier tot twintig seconden lang. 
...
%DIF < TODO figuur met verschillende activiteiten
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Gegevensverwerking: features berekenen}

Voor \DIFdelbegin \DIFdel{we classificatiemethodes kunnen gebruiken, moeten we eerst features berekenen}\DIFdelend \DIFaddbegin \DIFadd{er classificatiemethodes kunnen gebruikt worden, moeten eerst features berekend worden}\DIFaddend . Dit zijn parameters die \DIFdelbegin \DIFdel{we }\DIFdelend uit de samples van de accelerometer en gyroscoop \DIFdelbegin \DIFdel{kunnen halen}\DIFdelend \DIFaddbegin \DIFadd{gehaald kunnen worden}\DIFaddend . Om de verschillende features te berekenen, maken we gebruik van de MotionFingerprint tool~\cite{meert and schietgat:motionfingerprint}.
%DIF < TODO verwijzing naar tool 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < TODO uitleggen waarom we features berekenen
\DIFdelend 
...
Na het berekenen van de features \DIFdelbegin \DIFdel{hebben we voor }\DIFdelend \DIFaddbegin \DIFadd{heeft }\DIFaddend elke meting een set van parameters. Elke set vormt een instantie van de training set om een model uit te leren.
...
Met behulp van feature selectie onderzoeken we hoeveel features van het totaal aantal features van elke soort nodig zijn om een redelijke accuraatheid te behalen. Om de features met de meeste informatie te vinden, maken we gebruik van Weka's \emph{InfoGainAttributeEval} klasse\footnote{Meer informatie over deze klasse is te vinden op: \DIFdelbegin \DIFdel{http://weka.sourceforge.net/doc.dev/weka/attributeSelection/
InfoGainAttributeEval.html}\DIFdelend \DIFaddbegin \texttt{\DIFadd{http://weka.sourceforge.net/doc.dev/weka/}\\\DIFadd{attributeSelection/InfoGainAttributeEval.html}}\DIFaddend }.
Deze klasse evalueert de waarde van elke feature met behulp van information gain.

%Evaluatie
Om vervolgens de geselecteerde features te evalueren\DIFaddbegin \DIFadd{, }\DIFaddend maken we gebruik van dubbele cross-validatie. Bij de eerste cross-validatie (10-fold) wordt de gegevensset telkens verdeeld in een training en test set. Voor elke training set \DIFdelbegin \DIFdel{selecteren we }\DIFdelend \DIFaddbegin \DIFadd{worden }\DIFaddend de beste features \DIFaddbegin \DIFadd{geselecteerd }\DIFaddend met de methode die hierboven werd beschreven, gebruik makend van een tweede cross-validatie (2-fold). Met de geselecteerde features wordt een model opgesteld met behulp van de Random Forests classificatiemethode in Weka. Het gevonden model wordt uiteindelijk ge\"evalueerd op de test set van de eerste cross-validatie.

%Resultaten
In figuur~\ref{fig:1} worden de resulaten voor de verschillende soorten features getoond. \DIFdelbegin \DIFdel{We zien dat we met }\DIFdelend \DIFaddbegin \DIFadd{Met }\DIFaddend enkel de statistische features \DIFdelbegin \DIFdel{al }\DIFdelend \DIFaddbegin \DIFadd{wordt }\DIFaddend een accuraatheid van 90\% \DIFdelbegin \DIFdel{behalen}\DIFdelend \DIFaddbegin \DIFadd{behaald}\DIFaddend . Zelfs met de helft van de features kan het model al met een accuraatheid van 80\% activiteiten voorspellen. We stellen vast dat van het grote aantal Fourier tranformatie features slechts een tiental nodig zijn om een nauwkeurigheid van 80\% te bekomen. De features die \DIFdelbegin \DIFdel{hierbij geselecteerd wordenzijn voornamelijk die voor lagere frequenties ($<2.5$ Hz) volgens het xy-vlak.  Wavelet transformatie featureslijken }\DIFdelend \DIFaddbegin \DIFadd{hier geselecteerd worden, zijn voornamelijk de de amplitudes van de versnelling en energie voor frequenties kleiner dan 2.5 Hz. In figuur~\ref{fig:8} zien we dat de versnellingen van de activiteiten niet meer dan twee grote pieken per seconde bevatten. Dat is een mogelijke verklaring voor het feit dat de amplitudes voor lagere frequenties de meeste informatie geven.  In tegenstelling tot de Fourier transformatie features, lijken de Wavelet transformatie features }\DIFaddend minder goed te werken met slechts 70\% accuraatheid. Tenslotte merken we in de laatste grafiek op dat de Hidden Markov Models enkel degelijk werken wanneer alle features gebruikt worden. \DIFaddbegin \DIFadd{De features zijn immers de log-likelihoods voor het Hidden Markov Model van elke activiteit. Om de activiteit van een meting te bepalen, wordt voor de gemeten versnellingen best de likelihood voor elk model berekend om de meting met elke activiteit te vergelijken. Het is echter wel belangrijk om op te merken dat de information gain van de HMM features voor niets doen, tanden poetsen en lift versnelt omhoog/omlaag lager ($<$ 0.6) is dan voor de andere activiteiten ($>$ 1.8). Deze vier activiteiten hebben kleine versnellingen en kunnen daardoor gemakkelijk met elkaar verward worden. Zo hebben metingen voor niets doen en tanden poetsen een hoge likelihood voor de modellen van deze vier activiteiten. Bij metingen voor lift omhoog en omlaag zijn de likelihoods steeds voor beide activiteiten hoog. Dat heeft als gevolg dat de features voor deze vier activiteiten minder informatie geven dan de andere features.
}\DIFaddend 
...
Om de methodes te evalueren, \DIFdelbegin \DIFdel{maken we gebruik }\DIFdelend \DIFaddbegin \DIFadd{wordt er gebruik gemaakt }\DIFaddend van de Weka Machine Learning Toolkit~\cite{waikato:weka}.
Die bevat classifiers die de hierboven vermelde methodes implementeren. \DIFdelbegin \DIFdel{Voor beslissingsbomen maken we gebruik van de }\DIFdelend \DIFaddbegin \DIFadd{De }\DIFaddend J48 classifier \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{wordt gebruikt voor beslissingsbomen, dit is }\DIFaddend een implementatie van het C4.5 algoritme.
Verder \DIFdelbegin \DIFdel{gebruiken we IBk }\DIFdelend \DIFaddbegin \DIFadd{wordt IBk gebruikt }\DIFaddend voor k-Nearest Neighbours en LibSVM voor Support Vector Machines.
...
wordt de accuraatheid van de verschillende classificatiemethodes vergeleken. We zien dat RandomForest de hoogste accuraatheid (\DIFdelbegin \DIFdel{93,75}\DIFdelend \DIFaddbegin \DIFadd{93.75}\DIFaddend \%) geeft voor de metingen. Ook J48 geeft een goede accuraatheid (87.5\%). De hogere accuraatheid van Random Forests ten opzichte van beslissingsbomen is te verklaren door het feit dat Random Forests minder kans op overfitting heeft\DIFdelbegin \DIFdel{.~\mbox{%DIFAUXCMD
\cite{breiman:randomforests}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{breiman:randomforests}
}%DIFAUXCMD
. }\DIFaddend Naive Bayes lijkt ook goed te werken, terwijl met IBk en vooral LibSVM een lagere accuraatheid bereikt wordt. De accuraatheid van de methodes kan echter wel verbeterd worden door de parameters te optimaliseren.
...
%DIFDELCMD <     ymin=0,
%DIFDELCMD <     xticklabels from table={Data/methodes.dat}{methode},
%DIFDELCMD <     xticklabel style={align=center},
%DIFDELCMD <     nodes near coords,
%DIFDELCMD <     ylabel={Accuraatheid (\%)}
%DIFDELCMD < ]
%DIFDELCMD < 
...
\DIFdel{Data/methodes.dat}%DIFDELCMD < }%%%
\DIFdel{;
}%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{Voor }\DIFdelend \DIFaddbegin \DIFadd{Figuur \ref{fig:confusionmatrix} toont een confusion-matrix voor }\DIFaddend het model dat \DIFaddbegin \DIFadd{geleerd werd }\DIFaddend door Random Forests\DIFdelbegin \DIFdel{geleerd werd, hebben we in figuur \ref{fig:confusionmatrix} een confusion-matrix geplot}\DIFdelend . Het valt op dat activiteiten met grote versnellingen -- wandelen, lopen, fietsen en springen -- goed te herkennen zijn. Alle metingen voor deze activiteiten worden immers correct geclassificeerd door het model. Niets doen, lift versnelt omhoog/omlaag en tanden poetsen hebben kleinere versnellingen dan de andere activiteiten. We zien dat twee metingen van niets doen als tanden poetsen worden geclassificeerd. Ook lift omhoog en omlaag worden gemakkelijk met elkaar verward. Omwille van de kleine versnellingen zijn de verschillen tussen de activiteiten subtieler, wat een verklaring kan zijn waarom deze activiteiten moeilijker te herkennen zijn.
\\~\\
%korte conclusie
We kunnen voor de afzonderlijke activiteiten besluiten dat door Random Forests het nauwkeurigste (\DIFdelbegin \DIFdel{93,75}\DIFdelend \DIFaddbegin \DIFadd{93.75}\DIFaddend \%) model geleerd wordt.
...
Om de metingen te evalueren, werd elke sequentie gelabeld. \DIFdelbegin \DIFdel{Elke label bevat }\DIFdelend \DIFaddbegin \DIFadd{De labels bevatten }\DIFaddend de start- en eindtijden van de verschillende activiteiten in de bijhorende meting. De delen die niet gelabeld werden, \DIFdelbegin \DIFdel{beschouwen we als ruis}\DIFdelend \DIFaddbegin \DIFadd{worden beschouwd als `ruis'}\DIFaddend . Ruis defini\"eren we als niet-herkenbare activiteit. Het gaat om periodes met activiteiten die niet tot de tien activiteiten behoren\DIFdelbegin \DIFdel{die we kunnen herkennen}\DIFdelend . De smartphone in de broekzak stoppen is hier een voorbeeld van.
...
In figuur~\ref{fig:3d} worden de verschillende combinaties van de \DIFdelbegin \DIFdel{tijdvenster- en overlappingsgrootte }\DIFdelend \DIFaddbegin \DIFadd{tijdsvenstergrootte en het overlappingspercentage }\DIFaddend getoond met de behaalde accuraatheid. Indien \DIFdelbegin \DIFdel{we de x-as volgen (door }\DIFdelend de overlapping constant \DIFdelbegin \DIFdel{te houden}\DIFdelend \DIFaddbegin \DIFadd{wordt gehouden (figuur~\ref{fig:seq2d}}\DIFaddend ), zien we dat naarmate de tijdsvenstergrootte stijgt, ook de accuuraatheid stijgt. Er wordt een maximum gehaald rond de vier tot zes seconden, daarna daalt de accuuraatheid terug. Bij \DIFdelbegin \DIFdel{het volgen van de y-as (met een constante tijdsvenstergrootte ), }\DIFdelend \DIFaddbegin \DIFadd{een vaste tijdsvenstergrootte }\DIFaddend constateren we dat hoe groter het overlappingspercentage wordt, hoe nauwkeurigere voorspellingen gemaakt kunnen worden.

%DIF < TODO 
\begin{figure}[h]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\textwidth]{figures/3dplot}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
  ...
  \begin{subfigure}[b]{.99\linewidth}
    ...
    \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{3D-plot dat de resultaten toont van het experiment (besproken in sectie 3.4). Op de x-as wordt de venstergrootte geplot in seconden. Op y-as zijn de overlappingen te zien in procenten. Uitvoering van het algoritme met de combinatie }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Accuraatheid voor verschillende combinaties }\DIFaddendFL van \DIFdelbeginFL \DIFdelFL{deze parameters resulteert in een accuraatheid (geplot op de z-as)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{tijdvenstergrootte en overlapping}\DIFaddendFL }\label{fig:3d}
  \DIFaddbeginFL \end{subfigure}

  \begin{subfigure}[b]{.99\linewidth}
    \centering
    \includegraphics[width=0.99\textwidth]{figures/constantoverlap}
    \caption{\DIFaddFL{Met een constante overlapping (van 75}\% \DIFaddFL{op deze figuur) wordt de hoogste accuraatheid bereikt bij een tijdsvenstergroote van ongeveer vier tot zes seconden.}}\label{fig:seq2d}
  \end{subfigure}

  \caption{\DIFaddFL{Resultaten van het experiment voor de parameters van Algoritme~\ref{alg:sequentie}}}
...
%DIFDELCMD < %%%
\DIFdel{Uit het vorige experiment kunnen we concluderen dat }\DIFdelend \DIFaddbegin \\\DIFadd{~}\\
\DIFadd{We kunnen concluderen dat voor het algoritme }\DIFaddend de optimale venstergrootte vier tot zes seconden is\DIFaddbegin \DIFadd{, }\DIFaddend in combinatie met een overlappingspercentage van 75\%. Deze waarden samen met 50\% ruis cut-off leveren een gemiddelde nauwkeurigheid van 84\% voor de opgemeten sequenties.
\DIFdelbegin %DIFDELCMD < \\%%%
\DIFdel{~}%DIFDELCMD < \\
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF < Korte conclusie
\DIFdel{Voor de sequentie van activiteiten kunnen we concluderen dat voor ons algoritme de optimale venstergrootte vier tot zes seconden is in combinatie met 75}%DIFDELCMD < \% %%%
\DIFdel{overlapping. Er wordt een gemiddelde accuraatheid behaald van 84}%DIFDELCMD < \%
%DIFDELCMD < %%%
\DIFdelend 
...
Het doel van dit onderzoek was om een model te vinden om afzonderlijke bewegingen te herkennen. Vervolgens gebruikten we dit model om een sequentie van verschillende activiteiten te evalueren.
	...
	In sectie 3 werden sequenties van activiteiten ge\"evalueerd. Hiervoor hebben we een algoritme ontwikkeld waarvan de pseudocode besproken werd in sectie 3.3. \DIFdelbegin %DIFDELCMD < 
...
\DIFaddbeginFL \caption{\DIFaddFL{De tien verschillende activiteiten die afzonderlijk opgemeten werden voor dit onderzoek. Hierbij moet opgemerkt worden dat versnelling (y-as) niet op elke figuur dezelfde stap heeft. Zo hebben de activiteiten die getoond worden op figuur (g) t.e.m. (j) een versnelling kleiner dan }\'e\'en\DIFaddFL{.}}

  \DIFaddendFL \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.99\textwidth]{figures/wandelen}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.80\textwidth]{figures/wandelen}
    \DIFaddendFL \caption{Wandelen}\label{fig:1a}
  \end{subfigure}% 
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/lopen}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/lopen}
    \DIFaddendFL \caption{Lopen}\label{fig:1b}
  \end{subfigure} \\~\\
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/springen}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/springen}
    \DIFaddendFL \caption{Springen}\label{fig:1c}
  \end{subfigure}
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/fietsen}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/fietsen}
  ...
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/trapop}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/trapop}
  ...
  \begin{subfigure}[b]{.49\linewidth}
    \centering 
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/trapaf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/trapaf}
  ...
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/liftau}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/liftau}
  ...
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/liftad}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/liftad}
  ...
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/tandenpoetsen}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.80\textwidth]{figures/tandenpoetsen}
  ...
  \begin{subfigure}[b]{.49\linewidth}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.99\textwidth]{figures/nietsdoen}
%DIFDELCMD <     %%%
